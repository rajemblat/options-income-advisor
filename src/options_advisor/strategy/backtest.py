...
class HistoricalMoveCheck:
    """Sección 'check histórico' (pedido 2026-07-28): de todas las ventanas de `window_days`
    días CALENDARIO posibles en los últimos ~5 años de barras diarias reales, cuántas veces el
    precio se movió al menos tanto como necesitaría moverse HOY para llegar al strike de esta
    alerta puntual. Es análisis HISTÓRICO — lo que pasó antes no garantiza que no vuelva a
    pasar (ni que sí), el mercado puede comportarse distinto en el futuro."""
...
def historical_move_frequency(
    ...
    for i, start_bar in enumerate(bars):
        ...
        total_windows += 1
        if actual_move_pct >= required_move_pct:
            occurrences += 1
